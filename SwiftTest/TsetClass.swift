...
//属性
//概述:包括存储属性  计算属性 类型属性 以及 全局变量和局部变量
...
// 属性将 值 与 类 结构 或枚举 关联. 在oc中 常用的是类
//存储属性 存储 常量或变量作为实例的一部分,而计算属性计算一个值.
//计算属性可以用于类 结构体 枚举 , 存储属性 只有 前两者
//存储属性和计算属性 通常与 特定的类型的实例关联 但是属性也可以直接作用于类型本身,这种属性称为 类型属性
//还可以定义属性观察器来监控属性值的变化,以此来触发一个自定义的操作.
//属性观察器 可以添加到自己定义的存储属性上 也可以添加到从父类继承的属性上
